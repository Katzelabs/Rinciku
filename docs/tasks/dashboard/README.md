# dashboard — tasks

Read-only monthly overview: income, spent so far, days remaining in cycle, tier breakdown, and a budget-health indicator. Depends on `expenses/` and `essentials/` APIs being in place.

| # | Task | Status |
|---|---|---|
| 01 | [dashboard-data-layer](01-dashboard-data-layer.md) | done |
| 02 | [dashboard-page](02-dashboard-page.md) | done |
| 03 | [needs-vs-wants-breakdown](03-needs-vs-wants-breakdown.md) | done |
| 04 | [budget-health-indicator](04-budget-health-indicator.md) | done |
| 05 | [income-received-vs-expected](05-income-received-vs-expected.md) | done |
